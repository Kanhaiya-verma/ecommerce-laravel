    ...
    value: function drawPoint(x, y, radius, finishRadius, realIndex, dataPointIndex, j) {
      ...
      if (w.globals.dataChanged) {
        if (this.dynamicAnim) {
          ...
          var prevPathJ = null;
          prevPathJ = w.globals.Paths[realIndex] && w.globals.Paths[realIndex][j];
...
function () {
  ...
  _createClass(Bar, [{
    ...
    /** getPathFrom is a common function for bars/columns which is used to get  paths when data changes.
     * @memberof Bar
    ...
    value: function getPathFrom(realIndex, j) {
      ...
      for (var pp = 0; pp < w.globals.Paths.length; pp++) {
        var gpp = w.globals.Paths[pp];

        if (gpp.paths.length > 0 && parseInt(gpp.realIndex) === parseInt(realIndex)) {
          if (typeof w.globals.Paths[pp].paths[j] !== 'undefined') {
            pathFrom = w.globals.Paths[pp].paths[j].d;
          }
        }
      }
        ...
        for (var j = 0; j < heatSeries[i].length; j++) {
          ...
          if (w.globals.dataChanged) {
            ...
            if (this.dynamicAnim.enabled && w.globals.shouldAnimate) {
              ...
              var colorFrom = w.globals.Paths[i] && w.globals.Paths[i][j] && w.globals.Paths[i][j].color;
  ...
  _createClass(Pie, [{
    ...
    value: function draw(series) {
      ...
      if (w.globals.dataChanged) {
        ...
        for (var _k = 0; _k < w.globals.Paths.length; _k++) {
          // CALCULATE THE PREV TOTAL
          prevTotal += Utils.negToZero(w.globals.Paths[_k]);
        }

        var Angle;

        for (var _i = 0; _i < w.globals.Paths.length; _i++) {
          // CALCULATE THE  ANGLES
          Angle = this.fullAngle * Utils.negToZero(w.globals.Paths[_i]) / prevTotal;
          this.prevSectorAngleArr.push(Angle);
        }
      }
  ...
  _createClass(Radar, [{
    ...
    value: function getPathFrom(realIndex) {
      ...
      for (var pp = 0; pp < w.globals.Paths.length; pp++) {
        ...
        if (gpp.paths.length > 0 && parseInt(gpp.realIndex) === parseInt(realIndex)) {
          if (typeof w.globals.Paths[pp].paths[0] !== 'undefined') {
            pathFrom = w.globals.Paths[pp].paths[0].d;
          }
        }
      }
    ...
    value: function drawArcs(opts) {
      ...
      for (var i = reverseLoop ? opts.series.length - 1 : 0; reverseLoop ? i >= 0 : i < opts.series.length; reverseLoop ? i-- : i++) {
        ...
        if (w.globals.dataChanged) {
          ...
          prevEndAngle = Math.round(totalAngle * Utils.negToZero(w.globals.Paths[i]) / 100) + prevStartAngle;
        }
...
function () {
  ...
  _createClass(Line, [{
    ...
    key: "checkPaths",
    value: function checkPaths(_ref3) {
      ...
      for (var pp = 0; pp < w.globals.Paths.length; pp++) {
        ...
        if ((gpp.type === 'line' || gpp.type === 'area') && gpp.paths.length > 0 && parseInt(gpp.realIndex) === parseInt(realIndex)) {
          if (gpp.type === 'line') {
            ...
            pathFromLine = w.globals.Paths[pp].paths[0].d;
          } else if (gpp.type === 'area') {
            this.appendPathFrom = false;
            pathFromArea = w.globals.Paths[pp].paths[0].d;

            if (w.config.stroke.show) {
              pathFromLine = w.globals.Paths[pp].paths[1].d;
            }
          }
        }
      }
    ...
    value: function determineFirstPrevY(_ref4) {
      ...
      if (typeof series[i][0] !== 'undefined') {
        ...
        if (w.config.chart.stacked && i > 0 && typeof series[i][0] === 'undefined') {
          ...
          for (var s = i - 1; s >= 0; s--) {
            // for loop to get to 1st  value until we get it
    ...
    value: function getPaths() {
      ...
      if (heatmapColors.length > 0) {
        for (var h = 0; h < heatmapColors.length; h++) {
          ...
          w.globals.Paths.push(_dArr2);
        }
      }

      if (!w.globals.axisCharts) {
        // for non-axis charts (i.e., circular charts, pathFrom is not usable. We need whole series)
        w.globals.Paths = w.globals.series;
      }
    }
  }, {
    key: "handleNoData",
...
function (_Toolbar) {
  ...
  _createClass(ZoomPanSelection, [{
    ...
    } // remove the event listeners which were ly added on hover area
...
function arcToBeziere(pos, val) {
    ...
    // This is done by multiplying the ratios calculated ly by the distance between
...
function () {
  ...
  _createClass(ApexCharts, [{
     ...
     * This function fixes an important bug where a user might load a new series after zooming in/out of  series which resulted in wrong min/max
